...
public class Primes
{
    ...
    public static void main(String args[])
    {
        ...
        ListInterface<Integer> com=new AList(max);
        ListInterface<Integer> can=new AList(max);
        ListInterface<Integer> prim=new AList(max);
        for(int i=2;i<=max;i++) {
        	can.add(i);
        }
        while(!can.isEmpty()) {
        	int no=can.remove(1);
        	prim.add(no);                 //to add the no to the prime list
        	getComposites(can,com,no);    //to add all its multiple to composite no.
        }
        System.out.println("Candidates:-"+can);
        System.out.println("Composites:-"+com);
        System.out.println("Primes:-"+prim);
    }
    ...
    public static void getComposites(ListInterface<Integer> candidates, ListInterface<Integer> composites, Integer prime)
    {
        for(int i=1;i<=candidates.getLength();i++) {
        	...
        	{
        		composites.add(candidates.remove(i));   //to add composites to the respective list
        		i--; 
        	}
        }
    }
